...
class Program
{
    ...
    static int[,] LerMatrizUsuario(int n = 5) {
        ...
        for (int i = 0; i < n; i++) {
            for (int j = i+1; j < n; j++) {
                int dist = LerDistUsuario(i+1, j+1);
                matriz[i,j] = dist;
                matriz[j,i] = dist;
            }
        }
    ...
    static int[] LerPercursoUsuario(int n = 5) {
        ...
        int[] percurso = textoPercurso.Select(x => (int.TryParse(x, out int y) ? y-1 : -1)).ToArray();
        while (percurso.Any(x => x < 0 || x >= n)) {
            Console.WriteLine("Erro na leitura.");
            Console.WriteLine($"Insira o percurso com as cidades separadas por espaço:");
            textoPercurso = (Console.ReadLine() ?? "").Split(" ");
            percurso = textoPercurso.Select(x => (int.TryParse(x, out int y) ? y-1 : -1)).ToArray();
        }
        return percurso;
    }
